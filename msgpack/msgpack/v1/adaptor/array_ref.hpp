...
struct convert<msgpack::type::array_ref<T> > {
    msgpack::object const& operator()(msgpack::object const& o, msgpack::type::array_ref<T>& v) const {
        if (!v.data) { ExRaiseStatus(EMSGPACK_TYPE_ERROR); }
        if (o.type != msgpack::type::ARRAY) { ExRaiseStatus(EMSGPACK_TYPE_ERROR); }
        if (v.size() < o.via.bin.size) { ExRaiseStatus(EMSGPACK_TYPE_ERROR); }
...
template <typename T>
struct convert<msgpack::type::array_ref<eastl::vector<T> > > {
    msgpack::object const& operator()(msgpack::object const& o, msgpack::type::array_ref<eastl::vector<T> >& v) const {
        if (!v.data) { ExRaiseStatus(EMSGPACK_TYPE_ERROR); }
        if (o.type != msgpack::type::ARRAY) { ExRaiseStatus(EMSGPACK_TYPE_ERROR); }
        ...
        if (o.via.array.size > 0) {
            msgpack::object* p = o.via.array.ptr;
            msgpack::object* const pend = o.via.array.ptr + o.via.array.size;
            typename eastl::vector<T>::iterator it = v.data->begin();
            do {
                p->convert(*it);
                ++p;
                ++it;
