...
void* DoGetProcAddressA(void* base, const char* name) {
	void* fptr = NULL;

	if (base == &dummy_module) {
		ANSI_STRING as;
		UNICODE_STRING us;

		RtlInitAnsiString(&as, name);
		RtlAnsiStringToUnicodeString(&us, &as, TRUE);
		fptr = MmGetSystemRoutineAddress(&us);
...
void* DoNewPage(SIZE_T size) {
	return ExAllocatePoolWithTag(NonPagedPool, size, 'ffi');
}

void DoFreePage(void* base, SIZE_T size) {
	ExFreePool(base);
}
...
static int cdata_tostring(lua_State* L)
{
    ...
    case INT64_TYPE:
        ksprintf(buf, 64, ct.is_unsigned ? "%"PRIu64 : "%"PRId64, *(uint64_t*) p);
        lua_pushstring(L, buf);
        return 1;

    default:
        ksprintf(buf, 64, ct.is_unsigned ? "%"PRId64 : "%"PRId64, (int64_t) check_intptr(L, 1, p, &ct));
        lua_pushstring(L, buf);
        return 1;
    }
}
...
#ifdef _WIN32
#ifdef WINDDK
		jit->align_page_size = PAGE_SIZE - 1;
#else
        {
            SYSTEM_INFO si;
            GetSystemInfo(&si);
